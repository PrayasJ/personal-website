...
- Keep the existing portfolio design and structure; new features (including tools) must be additive and must not delete or replace current work; new tools should also appear in the projects list with links.
...
- Ads on tool pages should earn revenue without making the tools unusable; place them below the interactive stage so preview and download stay usable.
- New consumer SEO tools prioritize India everyday/commercial intent (photo KB caps, student utils, UPI/WhatsApp QR, rent/GST docs) over developer tools, games, or astrology; stay additive; do not invent official exam/gov size claims.
...
- Tools are registered centrally in `src/lib/tools.ts` (split modules for photo, PDF, calculators, student, QR, image, developer) and routed under `/tools`, `/calculators`, `/pdf`, `/image`, `/photo`, `/student`, and `/qr`; the `/tools` hub lists all categories with consumer-first ordering (Photo, PDF, Calculators, Student, QR, Image, Developer).
...
- Google AdSense is wired through `NEXT_PUBLIC_ADSENSE_*` environment variables and shown on tool pages; Vercel Speed Insights is integrated in the root layout.

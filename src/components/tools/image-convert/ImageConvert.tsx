...
import { canvasToBlob, extFor, fileToCanvas, type ImageFormat } from "@/lib/image";
...
export function ImageConvert() {
  ...
  async function preview() {
    ...
    try {
      ...
      setResult({
        blob,
        name: `${stem(file.name)}.${extFor(format)}`,
        src,
      });
    } catch (caught) {
      invalidate();
  ...
  return (
    <ToolShell>
      <p className="text-sm text-muted">
        PNG, JPEG, and WebP via the canvas encoder in this browser. GIF frames
        are not preserved — you get the first frame the browser draws. Preview
        the conversion, then confirm the download.
      </p>
      <div className="mt-4">
        <FileDrop
      ...
      />
      <Field label="Quality" htmlFor="img-convert-q" className="mt-4" hint="JPEG and WebP">
        <input
          id="img-convert-q"
          value={quality}
          onChange={(event) => {
      ...
      {file ? (
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          ...
          {result ? (
            <Stat label="Converted" value={formatBytes(result.blob.size)} tone="accent" />
          ) : null}
        </div>
      ) : null}
      <MediaWorkspace
        ...
        resultEmpty="Preview conversion to see the new format."
        sourceLabel="Original"
        resultLabel={result ? `Converted · ${formatBytes(result.blob.size)}` : "Converted"}
      />
      <div className="mt-4 flex flex-wrap gap-2">
        <Button variant="primary" disabled={busy || !file} onClick={preview}>
          {busy ? "Converting…" : "Preview"}
        </Button>
      ...
      <DownloadConfirm
        open={ask}
        filename={result?.name ?? "converted.webp"}

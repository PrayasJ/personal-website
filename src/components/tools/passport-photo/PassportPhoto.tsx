...
export function PassportPhoto() {
  ...
  const preset =
    passportPresets.find((item) => item.id === presetId) ?? passportPresets[0]!;
  ...
  async function preview() {
    ...
    try {
      ...
      const { blob } = await exportPassportPhoto(original, preset);
      const src = objectUrl(blob, resultUrl.current);
      resultUrl.current = src;
      setResult({
        blob,
        name: `${stem(file.name)}-${preset.width}x${preset.height}.jpg`,
        src,
      });
    } catch (caught) {
      invalidate();
  ...
  return (
    <ToolShell>
      <p className="text-sm text-muted">
        Center-crop to a common passport / form size, then download JPEG. These
        are common pixel sizes — not verified official exam or embassy rules.
        Always check the form you are filling.
      </p>
      <div className="mt-4">
        <FileDrop
      ...
      <Segmented
        ...
        options={passportPresets.map((item: PhotoPreset) => ({
          value: item.id,
          label: item.label,
        }))}
      />
      ...
      <MediaWorkspace
        source={source}
        result={output}
        sourceEmpty="Drop a portrait photo."
        resultEmpty="Preview to see the cropped size."
        sourceLabel="Original"
        resultLabel={
          result
            ? `${preset.width}×${preset.height} · ${formatBytes(result.blob.size)}`
            : "Result"
        }
      />
      <div className="mt-4 flex flex-wrap gap-2">
        <Button variant="primary" disabled={busy || !file} onClick={preview}>
          {busy ? "Cropping…" : "Preview"}
        </Button>
      ...
      <DownloadConfirm
        open={ask}
        filename={result?.name ?? "passport.jpg"}
